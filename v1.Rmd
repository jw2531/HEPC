...
ggplot(meanByMonth)+geom_point(aes(YearMon,ave),size=5)+ggtitle("Monthly Mean")+facet_wrap(~key,scales="free_y",nrow=4)
```
...
```{r}
meanByYear%>%filter(Year>=2007)%>%group_by(Year)%>%dplyr::summarise(tot=sum(ave))%>%ggplot()+geom_point(aes(Year,tot),size=5)+ggtitle("Yearly Mean")+xlim(2007,2010)
meanByYear%>%filter(Year>=2007)%>%ggplot()+geom_point(aes(Year,ave),size=5)+ggtitle("Yearly Mean")+facet_wrap(~key,scales="free_y",nrow=2)+xlim(2007,2010)
meanByYear_pct%>%filter(Year>=2007)%>%ggplot()+geom_point(aes(Year,ave_pct),size=5)+ggtitle("Yearly Mean")+facet_wrap(~key,scales="free_y",nrow=2)+xlim(2007,2010)
```
...
* With the domiant categories of air-conditioninig+water heater and the other compensating each other, the overall yearly mean decreased. 
